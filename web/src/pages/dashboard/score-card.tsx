          ...
          <div className="grid place-items-center gap-3">
            <div className="relative" style={{ width: SIZE, height: SIZE }}>
              ...
              <div className="absolute inset-0 grid place-items-center">
                <div className="text-center">
                  <p className="num-tabular text-3xl font-bold leading-none">
            ...
            <div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-4">
              {(['efficiency', 'profit', 'safety', 'consistency'] as const).map((k) => (
                <div key={k} className="rounded-md border border-border/60 bg-card/50 px-2 py-1.5 text-center">
                  <p className="text-[10px] uppercase tracking-wide text-muted-foreground">{k}</p>
                  <p className="num-tabular text-sm font-semibold">{formatNumber(data[k] ?? 0, locale)}</p>
                </div>
              ))}
            </div>
          </div>
